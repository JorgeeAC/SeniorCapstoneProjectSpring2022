    ...
    def token_backend(self):
        if self._token_backend is None:
            self._token_backend = import_string(
                "wrench_project.jwt_stuff.state.token_backend"
            )
...
class BlacklistMixin:
    """
    If the `wrench_project.jwt_stuff.token_blacklist` app was configured to be
    ...
    if "wrench_project.jwt_stuff.token_blacklist" in settings.INSTALLED_APPS:
